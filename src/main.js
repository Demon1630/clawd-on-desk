...
const { getCodexQuotaStatus } = require("./codex-quota-service");
...
const { extractLastAssistantTextFromTranscript } = require("../hooks/codex-assistant-output");
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    createWindow();
    ipcMain.handle("codex-quota:get", async () => getCodexQuotaStatus());
    ipcMain.handle("codex-assistant-output:get", async (_event, transcriptPath) => {
      if (typeof transcriptPath !== "string" || !transcriptPath.trim()) return null;
      return extractLastAssistantTextFromTranscript(transcriptPath);
    });
