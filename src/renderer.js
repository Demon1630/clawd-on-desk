...
const quotaBadgeEl = document.getElementById("codex-quota-badge");
const quotaPrimaryEl = document.getElementById("codex-quota-primary");
const quotaSecondaryEl = document.getElementById("codex-quota-secondary");
const petStepCardEl = document.getElementById("codex-step-card");
...
function computeQuotaPaceWarning(window) {
  ...
  if (!Number.isFinite(resetMs) || !Number.isFinite(durationMs) || durationMs <= 0) return null;

  const remainingMs = Math.max(0, resetMs - Date.now());
  const expectedUsed = clampPercent((1 - (remainingMs / durationMs)) * 100);
  const actualUsed = clampPercent(window.usedPercent);
...
function formatQuotaWindow(label, window, fallbackRemainingPercent, includeReset = false) {
  const remainingPercent = Number.isFinite(window && window.remainingPercent)
    ? window.remainingPercent
    : (Number.isFinite(fallbackRemainingPercent) ? fallbackRemainingPercent : null);
  const parts = [remainingPercent == null ? label + " --" : label + " " + remainingPercent + "%"];
  if (includeReset && window && window.resetsAt) {
    const resetAt = new Date(window.resetsAt);
...
function formatQuotaTitle(quota) {
  if (!quota || quota.status !== "ok") return "Codex quota status unavailable";
  const parts = [
    formatQuotaWindow("\u0035\u5c0f\u65f6", quota.primary, quota.remainingPercent, true),
    formatQuotaWindow("\u672c\u5468", quota.secondary, null, true),
  ].filter(Boolean);
  return parts.length ? parts.join(" | ") : "Codex quota status";
}

function renderQuotaBadge(quota) {
  if (!quotaBadgeEl || !quotaPrimaryEl || !quotaSecondaryEl) return;
  const status = quota && quota.status === "ok" ? quota : null;
  const primary = status && status.primary ? status.primary : null;
  const secondary = status && status.secondary ? status.secondary : null;
  const remaining = status && Number.isFinite(status.remainingPercent) ? status.remainingPercent : null;
  const state = !status
    ? "error"
    : remaining === 0
      ? "empty"
...
async function refreshCodexQuota() {
  if (!window.electronAPI || typeof window.electronAPI.getCodexQuota !== "function") {
    renderQuotaBadge({ status: "error", message: "Codex quota API unavailable" });
    return;
  }
  if (quotaRefreshInFlight) return quotaRefreshInFlight;
  quotaRefreshInFlight = window.electronAPI.getCodexQuota()
    .then((quota) => {
      renderQuotaBadge(quota || { status: "error", message: "Codex quota unavailable" });
      return quota;
    })
    .catch((err) => {
      renderQuotaBadge({ status: "error", message: err && err.message ? err.message : "Codex quota unavailable" });
      return null;
    })
    .finally(() => {
...
function startQuotaPolling() {
  ...
  quotaRefreshTimer = setInterval(refreshCodexQuota, 60000);
}

// 鈹€鈹€ Theme config (injected via preload.js additionalArguments) 鈹€鈹€
...
function initWithConfig(cfg) {
  ...
  // objectScale 鈥?applied via element.style in swapToFile() (CSP blocks <style> injection)
...
// 鈹€鈹€ Layered tracking state (multi-layer eye/head/body tracking) 鈹€鈹€
...
// monitor renders nothing there 鈥?the local display keeps the half-body peek.
//
...
function applyMiniClip(info) {
  ...
  if (info.edge === "left") {
    // Local display lies to the RIGHT of the seam 鈥?keep [f, 1], clip the left.
    clipLayer.style.clipPath = `inset(0 0 0 ${f * 100}%)`;
  } else {
    // Local display lies to the LEFT of the seam 鈥?keep [0, f], clip the right.
...
// is flipped 鈥?CSS animation transforms (float direction) are unaffected.
...
// 鈹€鈹€ Dual-channel rendering 鈹€鈹€
...
function swapToFile(file, state, useObjectChannel, options = {}) {
  ...
  if (useObj) {
    ...
    // once would reappear stuck on their last frame on subsequent loads 鈥?
...
// --- State change 鈫?switch animation (preload + instant swap) ---
window.electronAPI.onStateChange((state, svg) => {
  // Main process state change 鈫?cancel any active click reaction
  cancelReaction();
  ...
  // Different file 鈥?cancel pending, detach, and swap
  if (pendingNext) {
    if (pendingNext.tagName === "OBJECT") releaseObject(pendingNext);
    else releaseImg(pendingNext);
    pendingNext = null;
...
// 鈹€鈹€ Single-target eye tracking (legacy) 鈹€鈹€
...
// 鈹€鈹€ Layered tracking helpers 鈹€鈹€
...
function _startLayerAnimLoop() {
  ...
  function tick() {
    ...
    for (const layer of Object.values(_trackingLayers)) {
      ...
      // to this layer's range. No second normalization 鈥?tick.js already did it.
      const [tx, ty] = _getLayerTarget(layer, rawDx, rawDy);
...
// 鈹€鈹€ Attach / Detach (dispatches to correct system) 鈹€鈹€
...
startQuotaPolling();
startPetStepPolling();
window.addEventListener("focus", refreshCodexQuota);
window.addEventListener("focus", refreshPetStepCard);
function normalizePetStepText(value) {
  return String(value || "").replace(/\s+/g, " ").trim();
}

function petStateLabel(session) {
  if (!session) return "";
  if (session.badge === "done") return "\u5df2\u5b8c\u6210";
  if (session.badge === "interrupted") return "\u5df2\u4e2d\u65ad";
  switch (session.state) {
    case "thinking": return "\u6b63\u5728\u601d\u8003";
    case "working": return "\u6b63\u5728\u8fd0\u884c";
    case "juggling": return "\u6b63\u5728\u6574\u7406";
    case "notification": return "\u7b49\u5f85\u786e\u8ba4";
    case "sleeping": return "\u4f11\u7720\u4e2d";
    case "idle": return "\u5f85\u673a";
    default: return session.badge === "running" ? "\u6b63\u5728\u8fd0\u884c" : "\u5f85\u673a";
  }
}

function pickPetSession(snapshot) {
  const sessions = Array.isArray(snapshot && snapshot.sessions) ? snapshot.sessions : [];
  if (!sessions.length) return null;
  const preferredId = snapshot && (snapshot.hudLastSessionId || snapshot.lastSessionId);
  if (preferredId) {
    const found = sessions.find((session) => session && session.id === preferredId);
    if (found) return found;
  }
  const orderedIds = Array.isArray(snapshot && snapshot.orderedIds) ? snapshot.orderedIds : [];
  for (const id of orderedIds) {
    const found = sessions.find((session) => session && session.id === id && typeof session.assistantLastOutput === "string" && session.assistantLastOutput.trim());
    if (found) return found;
  }
  const anyOutput = sessions.find((session) => session && typeof session.assistantLastOutput === "string" && session.assistantLastOutput.trim());
  if (anyOutput) return anyOutput;
  const visible = sessions.find((session) => session && session.badge && session.badge !== "idle" && session.state !== "sleeping");
  return visible || null;
}

function renderPetStepCard(snapshot, liveOutput = null) {
  if (!petStepCardEl || !petStepTitleEl || !petStepBodyEl || !petStepStateEl) return;
  const session = pickPetSession(snapshot);
  if (!session) {
    petStepCardEl.hidden = true;
    return;
  }
  const title = normalizePetStepText(session.displayTitle || session.sessionTitle || session.id || "");
  const state = petStateLabel(session);
  const latest = normalizePetStepText(
    (liveOutput && liveOutput.text) || session.assistantLastOutput || state
  );
  petStepTitleEl.textContent = title || "Codex";
  petStepTitleEl.title = title || "Codex";
  petStepStateEl.dataset.state = session.badge || session.state || "idle";
  petStepStateEl.querySelector(".label").textContent = state;
  petStepBodyEl.textContent = latest || state;
  petStepBodyEl.title = latest || state;
  petStepCardEl.hidden = false;
}

async function refreshPetStepCard() {
  if (!window.electronAPI || typeof window.electronAPI.getSessionSnapshot !== "function") {
    renderPetStepCard(null);
    return;
  }
  if (petStepRefreshInFlight) return petStepRefreshInFlight;
  const currentToken = ++petStepRefreshToken;
  petStepRefreshInFlight = window.electronAPI.getSessionSnapshot()
    .then(async (snapshot) => {
      const currentSnapshot = snapshot || null;
      let liveOutput = null;
      const session = pickPetSession(currentSnapshot);
      if (session && session.transcriptPath && typeof window.electronAPI.getCodexLiveAssistantOutput === "function") {
        try {
          liveOutput = await window.electronAPI.getCodexLiveAssistantOutput(session.transcriptPath);
        } catch {}
      }
      if (currentToken === petStepRefreshToken) {
        renderPetStepCard(currentSnapshot, liveOutput);
      }
      return snapshot;
    })
    .catch(() => {
      if (currentToken === petStepRefreshToken) renderPetStepCard(null);
      return null;
    })
    .finally(() => {
      petStepRefreshInFlight = null;
    });
  return petStepRefreshInFlight;
}

function startPetStepPolling() {
  refreshPetStepCard();
  if (petStepRefreshTimer) clearInterval(petStepRefreshTimer);
  petStepRefreshTimer = setInterval(refreshPetStepCard, 1000);
}
